...
004 context-cost attribution, concise declaration development testing and negative/inconclusive confirmation are delivered; compact stays opt-in. Lifecycle/discovery hardening (025/029) is delivered. The [handoff](docs/handoff.md) now prioritizes 037 enterprise feasibility and the 040 date-time compatibility decision before a matched comparison. The earlier cancellation/read/executor/corpus/path-prefix sequences are delivered. QuickJS keeps Bun behind capability bridges; opt-in Bun retains ambient authority. The trial data show fewer calls at higher cost on this corpus, not the general superiority or failure of typed composition.

## Enterprise fit hypothesis

[Enterprise capability composition](docs/enterprise-capabilities.md) explores a different fit from local CLI replacement: existing OpenAPI/JSON Schema and MCP contracts can supply typed operation structure across unfamiliar customer services. Selective discovery keeps a large catalog out of model context; checked composition can then process the small relevant working set. Compare against equally lazy direct tools, not only an all-tools prompt. Contract versions and live authorization are separate: compile against an explicit schema, check current authority at every effect. The current prototype does not implement full dynamic enterprise policy/catalog lifecycle.

This hypothesis is captured in [035](.work/issues/035-enterprise-capability-scaling.md), with [primary-source research](docs/research/enterprise-capability-scaling.md). Accurate schemas may reduce onboarding work; they do not supply business semantics or establish runtime permission. Further 004 confirmation needs a separately frozen experiment. The selected enterprise work is 037; REST client generation in 039 remains an idea, though MCP/REST is the intended transport direction. Existing CLI/native adapters remain experimental scaffolding.

### Enterprise verification candidate

[Issue 037](.work/issues/037-enterprise-benchmark-reuse.md) has delivered AppWorld replay using the existing stdio MCP connector and upstream state grader, plus a separate BFCL invocation diagnostic. Response compatibility remains partial; no matched comparison is delivered. Compare equally lazy direct tools with typed composition and measure discovery separately. Catalog-size distractors and permission/schema changes are separate synthetic extensions, not properties already provided by Strata or official benchmark scores. See [AppWorld results](docs/appworld-spike.md), [BFCL results](docs/bfcl-diagnostic.md) and [candidate research](docs/research/tool-catalog-benchmarks.md).
