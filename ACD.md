...
This hypothesis is captured in [035](.work/issues/035-enterprise-capability-scaling.md), with [primary-source research](docs/research/enterprise-capability-scaling.md). Accurate schemas may reduce onboarding work; they do not supply business semantics or establish runtime permission. Independent confirmation of 004 remains the active context-cost trajectory. No enterprise implementation or model trial is started by this concept.

### Enterprise verification candidate

[Issue 037](.work/issues/037-enterprise-benchmark-reuse.md) proposes an offline AppWorld compatibility spike using the existing stdio MCP connector and upstream state grader. This tests whether existing schema-backed APIs reduce integration effort. Compare equally lazy direct tools with typed composition; measure discovery separately. Catalog-size distractors and permission/schema changes are separate synthetic extensions, not properties already provided by Strata or official benchmark scores. See [candidate research](docs/research/tool-catalog-benchmarks.md).
