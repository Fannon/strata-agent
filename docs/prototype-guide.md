# Prototype usage and technical reference

These are experimental setup and implementation details. Start with the [project overview](../README.md) for the research question, current findings and outlook.
...
* **Repository trials (repo-2):** four stages, 120 recorded cells; 116 exact answers and 114 overall successes. Typed profiles used fewer Pi calls but greater estimated cost/context. A 36-cell development run then showed −26.4% cost/success for compact versus full declarations (12/12 each); the 12-cell R-CALL confirmation was negative/inconclusive (stock 4/4, full 3/4, compact 2/4). See [reviewed report](repo-trials.md); pooled stages are not independent confirmation. Earlier $0.013 hybrid and $0.0117 engine pilots are historical feasibility runs.
...
Paid runs need explicit flags. See the [runner contract](benchmark.md) for budget reservations, result meanings, and limits:
...
Deep-dive: [docs/how-it-works.md](how-it-works.md) covers the full mechanism — run path, broker, checker, capabilities, executors, discovery, observability, and current trial evidence.
...
MCP metadata first becomes a protocol-independent `CapabilityModule`. This manifest feeds both declaration generation and runtime bindings. Implemented MCP, CLI-twin and native read-only repository connectors share the broker contract. Repository reads have best-effort resource checks; stronger resource policy and interactive grants remain future work.
...
QuickJS is the default executor, not a product requirement. The lasting interface is the typed capability layer spanning repository operations, APIs and MCP. Direct Bun is implemented opt-in (`STRATA_EXECUTOR=bun`) using the same contracts, semantic checker, permission-aware functions and instrumentation. Import checks alone do not establish containment. See [docs/executors.md](executors.md) and the [ACD](../ACD.md#permission-aware-functions-and-execution-alternatives).
...
- `src/capabilities/repo/`: native read-only repository connector (`readText`, `searchText`, `listFiles`, `gitStatus`, `gitLog`, `gitDiff`, `gitShow`) with best-effort path containment.
