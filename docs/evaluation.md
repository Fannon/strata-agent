...
The R-CALL family and the first -3/-4 held-out instances are now regression data: freeze new repository/workflow families before confirmatory evaluation. Plausible distractors are useful; ambiguity over multiple valid answers is a grading flaw, not desirable difficulty. Two repeats support exploratory diagnostics, not rankings. As reviewed 2026-09-16, 037 AppWorld replay and the BFCL diagnostic are implemented. AppWorld response compatibility remains partial (040 date-time decision); a task-solving replay and equally lazy direct-tool reference precede a matched pilot. Neither the unsuccessful AppWorld development run nor BFCL's corrected recorded-call verdicts establish comparative gains. See [AppWorld](appworld-spike.md), [BFCL](bfcl-diagnostic.md) and the [board](../.work/issues/index.md).
