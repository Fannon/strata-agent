...
- [x] Reproducibility manifest improvement: record actual selected task instances, fixture hashes and all backend versions. A source commit plus a whole-corpus definition list is useful provenance but not a complete environment snapshot.

  Delivered (no paid models, no runner redesign, historical artifacts untouched, additive fields only):
  `examples/benchmark/fingerprint.ts` (new, `FINGERPRINT_VERSION 2`) provides `collectEnvironment` (installed Pi/TypeScript/QuickJS/MCP versions from the actual `node_modules` package files plus Bun/Node/git/platform; missing packages record `{ version: null, error }`, never fabricated) and `fingerprintFixture` (stable task-material SHA256 over sorted POSIX-relative paths with exact bytes plus digests of the Git material components: `HEAD^{tree}`, `ls-files -s`, `status --porcelain`, `diff HEAD`, `diff --cached`). Git components are stored as SHA256 digests, never raw patches; a separate `exact` block records HEAD SHA plus refs/history digests for audit and is excluded from the top-level digest, so fresh builds fingerprint identically while history-only changes remain visible via `exact`. Excludes `.git/` contents and the root-level `.canary` only (a subdirectory `.canary` is material); the root canary is excluded from file walk AND Git index/status/diff inputs via a root-only pathspec. Git runs force `-c core.quotepath=false -c color.ui=false`, `--untracked-files=all`, `--no-color --no-ext-diff --no-textconv`, never user diff programs. Regular files and symlinks hash under distinct kind tags. Only ENOENT on `.git` means absent; any other I/O or Git failure throws visibly. `examples/repo-pilot.ts` (`artifactVersion` 2→3) writes `selectedTasks`/`selectedCells`/`repeats`, `environment`, `fixtureArtifact` (`<cell.id>/fixture.json`) and `fingerprintVersion` into `run-manifest.json`; whole-corpus `tasks` keeps id/ask only (no oracle, no build) with a `corpusNote`. No scratch rebuilds for top-level hashes: each built cell repository is fingerprinted and saved as `<cell.id>/fixture.json` BEFORE the randomized canary write and before the agent launches, so timeouts retain provenance. Tests: `test/integration/repo-manifest.test.ts` (8 tests) covers offline-runner selection output, real/missing versions, repeatability, content/Git-state change detection, root-canary exclusion (files and Git), subdirectory-canary materiality, symlink/file non-collision, volatile-rewrite stability with exact-identity movement, ENOENT-only absence and visible failure. Verification: `bun run check` passes; new file 8/8 pass; existing `repo-pilot`+`repo-tasks` suites pass; offline dry-run prints the selection matrix with no side effects. No live run, so end-to-end manifest output is not yet exercised.
  Limitations: version reads assume a `node_modules` layout; fingerprinting is POSIX-only like the runner.
  Live end-to-end (2026-09-13, author-paid): `bun examples/repo-pilot.ts --run --max-cost-usd 5 --tasks R-EXPORT-1 --profiles stock-pi,typed-quickjs --repeats 1` on the uncommitted 029/009 tree — 2/2 cells success with healthy harness, compliant policy and complete accounting; ledger $0.0022 of $5 cap. `run-manifest.json` carries artifactVersion 3 with selectedTasks/selectedCells/repeats/environment/fingerprintVersion and oracle-free corpus tasks (18 × id/ask); both per-cell `fixture.json` digests are identical (`5c7243ff…`, 7 files, `gitAbsentReason: no-git-repository` — R-EXPORT builds no git repo), confirming canary exclusion and cross-cell repeatability live. Raw artifacts remain local in `.work/repo-pilot/2026-09-13T14-32-30-392Z/`.
