...
| 3. Make comparison fair | 042 (partial), 045 delivered 2026-09-19 | Broker/direct-replay parity delivered; actual Pi entry-point compat forwarding fixed and regressed (real-entry parity green). No new comparison proposed or run. |
| 4. Measure value | 042 executed confounded, 045 delivered | v2 observed typed 9/18 strict at $0.0290/success vs direct 15/18 at $0.0069/success but confounded by entry-point validation mismatch — not a matched comparison, no proof of post-fix outcome. Keep v2 unchanged (no regrade/rerun). Any new comparison needs a separately proposed versioned bounded run with frozen tasks/settings/caps. |
...
Recommended next assignment: none selected. 045 is delivered (compat forwarding + real-entry regression green, no paid work). Before any new model comparison, propose separately a versioned bounded run with frozen matrix and spend cap; reused tasks remain development-only. Heavier-computation tasks, hybrid 044, REST and catalog scaling are alternatives requiring their own justification. Narrowing or stopping remains valid.
