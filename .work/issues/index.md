...
| 3. Make comparison fair | 042 (partial), 045 proposed | Broker/direct-replay parity delivered; actual Pi entry points mismatch (typed entry ignores compat setting direct path applies). 045 regression through real entry points required before any new comparison. |
| 4. Measure value | 042 executed confounded, 045 proposed | v2 observed typed 9/18 strict at $0.0290/success vs direct 15/18 at $0.0069/success but confounded by entry-point validation mismatch — not a matched comparison, no proof of post-fix outcome. Keep v2 unchanged (no regrade/rerun). Only after 045 parity, propose separately a versioned bounded comparison with frozen tasks/settings/caps. |
...
Recommended next assignment: [045](045-pilot-entrypoint-validation-parity.md), currently proposed. Reproduce and fix compatibility forwarding through the real typed/direct Pi entry points, preserving strict defaults and input checks. Finish deterministic parity checks before proposing any new model run. A subsequent comparison needs a separately frozen version, matrix and spend cap; reused tasks remain development-only. Heavier-computation tasks, hybrid 044, REST and catalog scaling are alternatives requiring their own justification. Narrowing or stopping remains valid.
...
On September 14 the user selected documentation reconciliation and the recommended enterprise sequence: AppWorld offline compatibility, matched lazy-direct versus typed pilot, separate catalog scaling, then confirmation only after a positive signal. Most work is delegated to Pi through OpenRouter with `meta/muse-spark-1.3-contributor`, medium reasoning; the supervisor scopes tasks, reviews changes and verifies results. The user explicitly permits sending this repository's contents to OpenRouter and allowing Pi to modify project files. Keep credentials and unrelated private material out of model context. The current cumulative ceiling is €5 for the entire program, including coding sessions; historical per-session $5 reservations are not additional authority. Review workers used smaller $2 worst-case reservation caps. Track measured usage across all sessions and reconcile currency and remaining allowance before another campaign; benchmark calls require a frozen matrix and spend cap. Record assumptions and results without erasing historical artifacts. Do not turn policy flags into answer failures or claim a clean heuristic audit proves containment.

## Recommended next work

| Issue | Current status |
| --- | --- |
| [037 — Reuse enterprise tool benchmarks](037-enterprise-benchmark-reuse.md) | gates 0–2 delivered for manual replay; pilot ran under 042; actual-entry compatibility gap now tracked in 045 |
| [040 — AppWorld date-time compatibility](040-appworld-datetime-compatibility.md) | validator/manual replay delivered; model-facing option propagation missing, tracked separately in 045 |
...
| [042 — Matched application comparison](042-matched-application-comparison.md) | v2 ran September 16; September 19 review found a validation-policy confound; observed typed 9/18 vs direct 15/18 preserved; no confirmation selected |
| [045 — Pilot entry-point validation parity](045-pilot-entrypoint-validation-parity.md) | PROPOSED: real-entry regression (synthetic datetime; explicit compat forwarding, strict defaults, inputs preserved); only after parity propose versioned bounded comparison; no paid work |
...
[035 — Enterprise discovery and typed composition](035-enterprise-capability-scaling.md) captures a potentially different use case: large dynamic API catalogs with small discovered working sets. It remains an unproven hypothesis and does not overturn 004's negative/inconclusive confirmation. [037 — Reuse enterprise tool benchmarks](037-enterprise-benchmark-reuse.md) delivered manual AppWorld feasibility, and 042 executed a pilot whose model-facing validation mismatch now needs 045. BFCL remains a complementary invocation diagnostic. Compare with equally lazy direct tools and test schema changes/revocation separately from task efficiency.
