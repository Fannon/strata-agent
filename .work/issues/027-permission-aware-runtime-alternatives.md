...
Status: implementation delivered and deterministically compared; matched model trials pending budget
...
- [ ] Run matched task comparisons only within selected budget; preserve contracts, checker, model and task data. Persistence/checking ablations remain separate.
...
Remaining: matched seeded-repository trials (stock Pi / typed-QuickJS /
typed-Bun, fixed model/settings, explicit spend cap) and the decision gate.
Needs concrete model-spend authorization; no paid runs were made here.
