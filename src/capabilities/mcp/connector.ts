...
export async function connectMcp(id: string, parameters: StdioServerParameters) {
  const client = new Client({ name: 'strata-agent', version: '0.1.0' });
  const transport = new StdioClientTransport({ ...parameters, stderr: 'ignore' });
  try {
    ...
    do {
      ...
      if (cursor && seen.has(cursor)) throw new Error('MCP tools/list repeated cursor');
    ...
    const connector: CapabilityConnector = {
      async invoke(operation, input, signal) {
        ...
        const result = await client.request({ method: 'tools/call', params: { name: operation, arguments: input } }, CallToolResultSchema, { signal });
        return { structured: result.structuredContent, untyped: result, isError: result.isError, rawBytes: bytes(result) };
      },
      close: () => client.close(),
    };
