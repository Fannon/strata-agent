...
export function validator(schema: JsonSchema) {
  ...
  return (value: unknown): string | undefined => check(value) ? undefined : ajv.errorsText(check.errors, { dataVar: 'value' });
}
...
export async function declarations(module: CapabilityModule): Promise<string> {
  ...
  for (const [i, op] of module.operations.entries()) {
    const input = `Input${i}`, output = `Output${i}`;
    const generate = (schema: JsonSchema, name: string) => compile(schema, name, {
      bannerComment: '', unknownAny: true, enableConstEnums: false,
